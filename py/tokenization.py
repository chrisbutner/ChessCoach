...
def ensure_tokenizer(config, vocabulary_size, log):
  model_path = config.join(config.training["commentary_path_training"], "tokenizer.model")
  try:
    ...
    vocabulary_path = config.join(config.training["commentary_path_training"], config.training["vocabulary_filename"])
