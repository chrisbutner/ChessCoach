...
def test_train(teacher):
  ...
  import numpy
  batch_size = network.config.network["batch_size"]
  ...
  #network.config.network["validation_interval"] = 1
  network.load_network(network.config.network["name"])
...
def test_train_commentary():
  ...
  from model import ModelBuilder
  import numpy
  batch_size = network.config.network["commentary_batch_size"]
  images = numpy.zeros((batch_size, ModelBuilder.input_planes_count), dtype=numpy.int64)
...
def create_save_training_network(teacher):
  ...
  network.networks.name = network.config.network["name"]
