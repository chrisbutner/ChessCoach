...
deployment_configs = {
  "selfplay6": {
    "roles": {
      "train": {
        "count": 1,
        "command": "docker run --rm --privileged --network host --mount type=bind,source=/usr/share/tpu,target=/usr/share/tpu --mount type=bind,source=/lib/libtpu.so,target=/lib/libtpu.so eu.gcr.io/chesscoach/chesscoach-train:selfplay8_v18",
        "on_error": "dmesg",
      },
      "play": {
        "count": 22,
        "command": "docker run --rm --privileged --network host --mount type=bind,source=/usr/share/tpu,target=/usr/share/tpu --mount type=bind,source=/lib/libtpu.so,target=/lib/libtpu.so eu.gcr.io/chesscoach/chesscoach-play:selfplay8_v18",
        "on_error": "dmesg",
      },
    },
  },
